...
namespace
{
	...
	const Animation::Desc desc_[] = {
		{"clap",
		  "data/Sample/FPS_Hands/FPS_Hands_v2.9x_animation.mv1",
		  1,
		  1.0f},
		{ "from_clap",
		  "data/Sample/FPS_Hands/FPS_Hands_v2.9x_animation.mv1",
...
bool SceneFps::Init()
{
	...
	{
		model_ =
			std::make_shared<Model>( "data/Sample/FPS_Hands/FPS_Hands_v2.9x.mv1"
			);
...
void SceneFps::Update()
{
	...
	//----------------------------------------------------------
	{
		...
		mat_world =
			mul( mat_world, matrix::rotateY( -90.0f * DegToRad )
			);	  // 方向調整
		mat_world =
			mul( mat_world, matrix::rotateX( 180.0f * DegToRad )
			);	  // 方向調整
		mat_world = mul(
			mat_world,
			matrix::translate( float3( -0.225f, 0.25f, 0.075f ) )
		);	  // 位置調整
...
void SceneFps::Draw()
{
	...
	//----------------------------------------------------------
	{
		// モデルに設定されているテクスチャを上書き
		model_knife_
			->overrideTexture( Model::TextureType::Diffuse, tex_kn0_albedo_ );
		model_knife_
			->overrideTexture( Model::TextureType::Albedo, tex_kn0_albedo_ );
		model_knife_
			->overrideTexture( Model::TextureType::Normal, tex_kn0_normal_ );
		model_knife_->overrideTexture(
		...
		model_knife_
			->overrideTexture( Model::TextureType::Diffuse, tex_kn1_albedo_ );
		model_knife_
			->overrideTexture( Model::TextureType::Albedo, tex_kn1_albedo_ );
		model_knife_
			->overrideTexture( Model::TextureType::Normal, tex_kn1_normal_ );
		model_knife_->overrideTexture(
			Model::TextureType::Roughness,
			tex_kn1_roughness_
		);
