...
void SceneEaseCurve::Draw()
{
	...
	auto drawMark = [&]( f32 t, u32 y, u32 color )
	{
		...
		u32 boxX = static_cast<u32>(
			lerp( float1( startX ), float1( endX - boxSize ), t )
		);
		u32 boxY = y;
	...
	//----------------------------------------------------------
	{
		...
		DrawString( startX - 8 - stringWidth, y, name, fontColor );	   // 左側 -
																	   // 右詰め
	...
	for( u32 i = 0; i < GetEaseFunctionMaxCount(); ++i )
	{
		...
		u32 stringWidth = GetDrawStringWidth(
			easeName[type],
			static_cast<int>( strlen( easeName[type] ) )
		);
		...
		DrawString(
			startX - 8 - stringWidth,
			y,
			easeName[type],
			fontColor
		);														 // 左側 - 右詰め
		DrawString( endX + 8, y, easeName[type], fontColor );	 // 右側 -
																 // 左詰め
