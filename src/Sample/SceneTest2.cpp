...
namespace Sample
{
		...
		public:
			...
			void Draw()
			{
				...
				DrawBox(
					x,
					y,
					x + static_cast<int>( HP_BAR_W * bar2_per ),
					y + HP_BAR_H,
					RED,
					TRUE
				);
				...
				DrawBox(
					x,
					y,
					x + static_cast<int>( HP_BAR_W * bar_per ),
					y + HP_BAR_H,
					SKYBLUE,
					TRUE
				);
	...
	public:
		...
		// 基本描画の後に処理します
		void PostDraw() override { HP::player_hp.Draw(); }
...
#else
		{
			auto obj = Scene::CreateObjectPtr<Object>( "Ground"
			);	  //->SetName( "Ground" );
			obj->AddComponent<ComponentModel>(
