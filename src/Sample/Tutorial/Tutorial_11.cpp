...
namespace Tutorial
{
	...
	public:
		...
		bool Init() override
		{
			...
			auto model = ground->AddComponent<ComponentModel>(
			...
			obj->AddComponent<ComponentModel>( "data/Sample/Player/model.mv1" )
				->SetAnimation( {
					{"idle",
					  "data/Sample/Player/Anim/Idle2.mv1",
					  0,
					  1.0f}, // Idle
					{ "jump",
					  "data/Sample/Player/Anim/Jump2.mv1",
					  0,
					  1.0f}	// Jump
			} );
			...
			boss->AddComponent<ComponentModel>( "data/Sample/Boss/model.mv1" )
				->SetAnimation( {
					{"idle",
					  "data/Sample/Player/Anim/Idle.mv1",
					  0,
					  1.0f}, // Idle
					{ "idle",
					  "data/Sample/Player/Anim/Walk.mv1",
					  0,
					  1.0f}, // Walk
					{ "jump",
					  "data/Sample/Player/Anim/Death.mv1",
					  0,
					  1.0f}	// Deah
			} );
			...
			cam_obj
				...
				->SetPositionAndTarget(
					{ 0, 0, 0 },
					{ 0, 0, 1 }
				);	  //< SpringArmとコリジョンを使う場合はこれで

			cam_obj->AddComponent<ComponentSpringArm>()
				->SetSpringArmRotate( { -45, 0, 0 }
				)	 //< X軸周りで回転させ45度の角度からObjectを見る
				->SetSpringArmLength( 30.0f
				)									   //< Objectとの距離 (オブジェクトにはカメラがついている)
			...
			cam_obj->AddComponent<ComponentCollisionSphere>()
				...
				->SetHitCollisionGroup(
					(u32
					)ComponentCollision::CollisionGroup::GROUND |	 // Ground
					(u32
					)ComponentCollision::CollisionGroup::PLAYER |	  // Player
