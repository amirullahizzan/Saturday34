...
namespace
{
	...
	class CharacterContactListener: public physics::Character::ContactListener
	{
		...
		{
			auto it = std::find(
				prop_body_id_list_.begin(),
				prop_body_id_list_.end(),
				other_body_id
			);
			if( it != prop_body_id_list_.end() )
			{
				size_t index =
					it - prop_body_id_list_.begin();	// 配列インデックス番号
...
bool ScenePhysicsLayer::Init()
{
	...
	physics::Engine::instance()
		->setLayerAlias( myLayerTable_, std::size( myLayerTable_ ) );
...
void ScenePhysicsLayer::Update()
{
	...
	//----------------------------------------------------------
	{
		...
		float3 new_velocity =
			character_
				->move( delta, move_direction, old_position, jump_direction );
...
void ScenePhysicsLayer::Draw()
{
	...
	//----------------------------------------------------------
	auto put_state =
		[]( const float3& position, const char* message, u32 color )
	{
		...
		s32 x = static_cast<s32>( screen_position.x ) -
				width / 2;										   // センタリング
	...
	put_state(
		rigid_body0_->position() + float3( 0.0f, 1.0f, 0.0f ),
		"押されない\n押せない",
		GetColor( 255, 255, 255 )
	);
	put_state(
		rigid_body1_->position() + float3( 0.0f, 1.0f, 0.0f ),
		"押されない\n押せる",
		GetColor( 255, 255, 255 )
	);
	put_state(
		rigid_body2_->position() + float3( 0.0f, 1.0f, 0.0f ),
		"押される\n押せる",
		GetColor( 255, 255, 255 )
	);
	put_state(
		rigid_body3_->position() + float3( 0.0f, 1.0f, 0.0f ),
		"押される\n押せない",
		GetColor( 255, 255, 255 )
	);

	put_state(
		rigid_body0_->position() + float3( 0.0f, 2.5f, 0.0f ),
		"当たらない",
		GetColor( 255, 64, 64 )
	);
	put_state(
		rigid_body1_->position() + float3( 0.0f, 2.5f, 0.0f ),
		"当たる",
		GetColor( 64, 255, 64 )
	);
	put_state(
		rigid_body2_->position() + float3( 0.0f, 2.5f, 0.0f ),
		"当たる",
		GetColor( 64, 255, 64 )
	);
	put_state(
		rigid_body3_->position() + float3( 0.0f, 2.5f, 0.0f ),
		"当たる",
		GetColor( 64, 255, 64 )
	);
...
void ScenePhysicsLayer::ResetPhysics()
{
	...
	// ボックス
	{
		...
		auto tex_albedo_ = std::make_shared<Texture>(
			path + "DefaultMaterial_Base_color.png"
		);
	...
	// ドラム缶
	{
		...
		auto tex_normal_ =
			std::make_shared<Texture>( path + "drum2_normal.png" );
		auto tex_roughness_ =
	...
	// 三角コーン
	{
		...
		auto tex_metalness_ = std::make_shared<Texture>(
			path + "Dirty_Traffic Cone_Metallic.png"
		);
		auto tex_ao_ =
			std::make_shared<Texture>( path + "Dirty_Traffic Cone_AO.png" );

		auto model =
			std::make_shared<Model>( "data/Sample/Traffic_Cone/Traffic Cone.mv1"
			);
	...
	rigid_body2_ = physics::createRigidBody(
		shape::Cylinder{ 0.5f, 0.375f },
		MyLayer::Prop2
	);
	rigid_body3_ = physics::createRigidBody(
		shape::ConvexHull( model_cone_.get() ),
		MyLayer::Prop3
	);
