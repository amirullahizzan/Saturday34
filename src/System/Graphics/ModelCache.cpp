...
ModelCache::ModelCache( std::string_view path )
{
	...
	// 　対応するキャッシュファイルのパスを取得
	{
		...
		GetTempPath(
			static_cast<DWORD>( sizeof( temporary_path ) ),
			temporary_path.data()
		);

		model_cache_path_ =
			std::string( temporary_path.data() ) + "BaseProject/" +
...
bool ModelCache::save( int mv1_handle ) const
{
	...
	stream.write(
		reinterpret_cast<char*>( &file_version ),
		sizeof( file_version )
	);
	...
	stream.write(
		reinterpret_cast<char*>( &vertex_count ),
		sizeof( vertex_count )
	);
	stream.write(
		reinterpret_cast<char*>( &index_count ),
		sizeof( index_count )
	);

	// 頂点配列
	stream.write(
		reinterpret_cast<char*>( varray.data() ),
		vertex_count * sizeof( VECTOR )
	);

	// インデックス配列
	stream.write(
		reinterpret_cast<char*>( iarray.data() ),
...
bool ModelCache::load()
{
	...
	//----------------------------------------------------------
	{
		auto handle = FileRead_fullyLoad_WithStrLen(
			model_cache_path_.data(),
	...
	//----------------------------------------------------------
	{
		...
		// 頂点バッファとインデックスバッファを作成
		handle_vb_ = CreateVertexBuffer(
			static_cast<s32>( varray.size() ),
			DX_VERTEX_TYPE_NORMAL_3D
		);
		handle_ib_ = CreateIndexBuffer(
