...
void Animation::update( f32 dt )
{
	...
	for( u32 i = 0; i < std::size( contexts_ ); ++i )
	{
		...
		// ブレンドを進める
		c.blend_ratio_ +=
			( ( 1.0f / blend_time_ ) * dt ) * ( ( i == 0 ) ? +1.0f : -1.0f );
	...
	for( u32 i = 0; i < std::size( contexts_ ); ++i )
	{
		...
		DxLib::MV1SetAttachAnimBlendRate(
			model_handle_,
			c.animation_attach_index_,
			ratio / blend_ratio_total
		);

		// 補間完了後は補間元のアニメーションを停止
		if( i > 0 )
		{
			if( c.blend_ratio_ == 0.0f )
...
)
{
	// ブレンドの速度
	blend_time_ =
		std::max( FLT_EPSILON, blend_time );	// 0.0fにならないように
...
bool Animation::attachAnimation( s32 context_index )
{
	...
	c.animation_attach_index_ = DxLib::MV1AttachAnim(
		model_handle_,
		animation_index,
		mv1_handle,
		true
	...
	// アニメーション総再生時間を取得
	c.animation_total_time_ = DxLib::MV1GetAttachAnimTotalTime(
		model_handle_,
		c.animation_attach_index_
	);
...
ResourceAnimation::ResourceAnimation( std::string_view path )
{
	...
	SetUseASyncLoadFlag( true );
	{
		// モデルの読み込み
		mv1_handle_ = DxLib::MV1LoadModel( resource_path.c_str() );
		DxLib::SetASyncLoadFinishCallback(
			mv1_handle_,
