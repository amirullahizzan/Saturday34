...
public:
	...
	//! (nullptrで無効化)
	void
	render( ShaderVs* override_vs = nullptr, ShaderPs* override_ps = nullptr );
...
private:
	...
	std::unique_ptr<DxLib_MV1MatrixCache>
				 mv1_matrix_cache_;					  //!< [DxLib] 行列キャッシュ
	...
	static inline std::shared_ptr<ShaderGs>
		shader_gs_streamout_position_;	  //!< ジオメトリシェーダー(頂点出力)
	static inline std::shared_ptr<ShaderGs>
		shader_gs_composite_prev_position_;	   //!< ジオメトリシェーダー(頂点合成)
	Microsoft::WRL::ComPtr<ID3D11GeometryShader>
		d3d_shader_gs_streamout_position_;	  //!< [D3D11]ストリーム出力用
											  //!< (頂点出力)
