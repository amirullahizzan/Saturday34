	...
	bool initialize( int mv1_handle )
	{
		...
		{	 // ダブルバッファの数
			...
			for( s32 t = 0; t < triangle_list_count; t++ )
			{
				...
				vertex_count *=
					3;	  // インデックスバッファを単純な三角形に展開した場合に最大3倍の頂点になる可能性
				{
					...
					D3D11_BUFFER_DESC desc{
						.ByteWidth =
							sizeof( float4 ) *
							vertex_count,	 // バッファーのサイズ (バイト単位)
						.Usage =
							D3D11_USAGE_DEFAULT,	// メモリの配置場所
													// (読み取りと書き込み方法)
						.BindFlags =
							D3D11_BIND_STREAM_OUTPUT |
							D3D11_BIND_SHADER_RESOURCE,	   // バインド用途
						.CPUAccessFlags =
							0,	  // CPUアクセス許可フラグ (殆どの場合0)
						.MiscFlags =
							D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS,	   // ByteAddressBuffer
						.StructureByteStride =
							0,	  // 構造化バッファの構造体あたりの幅
					};

					HRESULT hr = d3d_device->CreateBuffer(
						&desc,
						nullptr,
						&tlist.d3d_position_buffer_
					);
					if( hr != S_OK )
					{
						return false;
				...
				// StreamOutバッファ用のShaderResourceViewを作成(ByteAddressBuffer)
				{
					...
					desc.Format =
						DXGI_FORMAT_R32_TYPELESS;	 // float型
													 // (TYPELESS指定が必要)
					desc.ViewDimension =
						D3D11_SRV_DIMENSION_BUFFEREX;	 // BufferEx
					desc.BufferEx.FirstElement =
						0;	  // 最初の要素インデックス番号
					desc.BufferEx.Flags =
						D3D11_BUFFEREX_SRV_FLAG_RAW;	// ByteAddressBuffer
					desc.BufferEx.NumElements =
						vertex_count * 4;	 // float * 4 * 頂点数

					HRESULT hr = d3d_device->CreateShaderResourceView(
						tlist.d3d_position_buffer_.Get(),
						&desc,
						&d3d_srv
					);
					if( hr != S_OK )
...
bool Model::load( std::string_view path )
{
	...
	if( shader_vs_ == nullptr || shader_ps_ == nullptr )
	{
		...
		shader_vs_ = std::make_shared<ShaderVs>(
			"data/Shader/vs_model",
			VS_VARIANT_COUNT
		);

		// ピクセルシェーダー
		shader_ps_ = std::make_shared<ShaderPs>(
			"data/Shader/ps_model",
			PS_VARIANT_COUNT
		);
	...
	{
		shader_gs_streamout_position_ = std::make_shared<ShaderGs>(
			"data/Shader/gs_model_streamout_position",
			1
		);
		shader_gs_composite_prev_position_ = std::make_shared<ShaderGs>(
			"data/Shader/gs_model_composite_prev_position",
			1
		);

		// StreamOut用のバッファ出力レイアウト
		// ストリーム番号, "出力要素の型", 出力要素のインデックス,
		// 書き込みを開始するエントリのコンポーネント(0-1),
		// コンポーネントの数(1-4), ストリーム出力バッファースロット番号(0-3)
		...
		auto [shader_binary, shader_binary_size] =
			shader_gs_streamout_position_->shader_bytecode();
		...
		HRESULT hr = d3d_device->CreateGeometryShaderWithStreamOutput(
			...
			static_cast<u32>( std::size( so_declaration_entry )
			),				   // [in]  StreamOut用のバッファ出力レイアウト配列数
			strides.data(),	   // [in]  バッファーストライドの配列
			static_cast<u32>( strides.size()
			),								  // [in]  バッファーストライドの配列数
			D3D11_SO_NO_RASTERIZED_STREAM,	  // [in]
											  // ラスタライザーステージに送信されるストリームのインデックス番号
			nullptr,						  // [in]  ID3D11ClassLinkageのポインタ
			&d3d_shader_gs_streamout_position_
		);	  // [out] D3Dジオメトリシェーダー
...
)
{
	...
	if( overridedTextures_[static_cast<s32>( Model::TextureType::Specular )] )
	{
		SetUseTextureToShader(
			2,
			*overridedTextures_[static_cast<s32>( Model::TextureType::Specular
			)]
		);
	}
	...
	for( s32 mesh_index = 0;
		 mesh_index < MV1GetFrameMeshNum( mv1_handle_, frame_index );
		 ++mesh_index )
	{	 // フレームに含まれるメッシュの数
		s32 mesh = MV1GetFrameMesh(
			mv1_handle_,
			frame_index,
			mesh_index
		);	  // メッシュ番号

		for( s32 tlist_index = 0;
			 tlist_index < MV1GetMeshTListNum( mv1_handle_, mesh );
			 ++tlist_index )
		{	 // メッシュに含まれるトライアングルリストの数
			auto tlist = MV1GetMeshTList(
				mv1_handle_,
				mesh,
				tlist_index
			);	  // トライアングルリスト番号
			...
			auto material_index =
				MV1GetTriangleListUseMaterial( mv1_handle_, tlist );

			// 法線マップを使用しているかどうか
			bool use_normalmap =
				MV1GetMaterialNormalMapTexture( mv1_handle_, material_index ) !=
				-1;
			...
			auto vertex_type =
				MV1GetTriangleListVertexType( mv1_handle_, tlist );
			u32 variant_vs =
			...
			{
				auto* d3d_context = reinterpret_cast<ID3D11DeviceContext*>(
				...
				{
					ID3D11Buffer* stream_out_buffer =
						mv1_matrix_cache_->position_buffer( tlist );

					u32 offset[1] = { 0 };
					d3d_context->SOSetTargets( 1, &stream_out_buffer, offset );

					d3d_context
						->GSGetShader( &old_shader_gs, nullptr, nullptr );
					d3d_context->GSSetShader(
						d3d_shader_gs_streamout_position_.Get(),
						nullptr,
						0
					);
				}
...
void Model::on_initialize()
{
	...
	if( mv1_handle_ == -1 )
	{
		...
		// 行列キャッシュを初期化
		mv1_matrix_cache_ =
			std::make_unique<DxLib_MV1MatrixCache>( mv1_handle_ );
