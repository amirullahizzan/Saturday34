...
bool Texture::initialize( ID3D11Resource* d3d_resource )
{
	...
	//----------------------------------------------------------
	{
		...
		auto* d3d_texture_2d =
			reinterpret_cast<ID3D11Texture2D*>( d3d_resource );
		...
		{
			d3d_device
				->CreateRenderTargetView( d3d_resource, nullptr, &d3d_rtv_ );
		}
		...
		if( desc.BindFlags & D3D11_BIND_DEPTH_STENCIL )
		{
			...
			d3d_device
				->CreateDepthStencilView( d3d_resource, &view_desc, &d3d_dsv_ );
		}
	}
	return true;
}
