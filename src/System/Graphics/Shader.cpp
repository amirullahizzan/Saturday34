...
struct Variant
{
	...
	//! @param  [in]    dxlib_shader_type   [DxLib]
	//! シェーダーの種類(DX_SHADERTYPE_VERTEXなど)
	bool
	compile( const std::wstring& source_path, u32 index, int dxlib_shader_type )
	{
		//---------------------------------------------------------------------
		// ファイルからソースファイルを読み込み
		//---------------------------------------------------------------------
		...
		auto hr = D3DCompile(
			source_code.data(),	   // [in]  ソースコードのメモリ上のアドレス
		...
		// [DxLib] シェーダーを作成
		//------------------------------------------------------
		const void* shader_bytecode = byte_code->GetBufferPointer(
		);	  // シェーダーバイトコードの先頭アドレス
		auto shader_size = static_cast<int>( byte_code->GetBufferSize()
		);	  // シェーダーバイトコードのサイズ
...
struct Shader::Impl
{
	...
	//! 現在アクティブなシェーダーリスト
	static inline std::
		unordered_map<std::wstring, std::shared_ptr<Shader::Impl>>
			shaders_;
...
)
{
	...
	// 拡張子を除去したファイルパスを取得
	auto shader_path =
		path.parent_path().wstring() + L"/" + path.stem().wstring();
	...
	if( it == std::end( Shader::Impl::shaders_ ) )
	{	 // 見つからなかった場合は新規作成
		auto p = std::make_shared<Shader::Impl>(
			shader_path,
			dxlib_shader_type,
			variant_count
		);
...
{
	return variant_index < impl_->variant_.size()
			   ? impl_->variant_[variant_index].handle_
			   : -1;
}
...
//---------------------------------------------------------------------------
std::tuple<const void*, size_t> Shader::shader_bytecode( u32 variant_index
) const
{
