...
public:
	ObjTypeInfo(
		...
		size_t		 class_size,
		ObjTypeInfo* parent_type = nullptr,
...
#define BP_OBJECT_DECL( CLASS, ... )                                                                           \
	using Super =                                                                                              \
		ObjClass; /*! 親クラスの型 :                                                                     \
					 上のクラス階層で定義されているClassを使うことで親クラスを定義 \
				   */                                                                                          \
	using ObjClass =                                                                                           \
		CLASS; /*! 自クラスの型 : これ以降は親クラスではなく自クラスを指す */        \
                                                                                                               \
	/*! 型情報 */                                                                                           \
	static inline ClassObjectType<CLASS> Type =                                                                \
		ClassObjectType<CLASS>( #CLASS, &Super::Type, __VA_ARGS__ );                                           \
                                                                                                               \
	/*! 型情報を取得 */                                                                                  \
...
#define BP_OBJECT_BASE_TYPE( CLASS )                                                       \
	using MyClass = CLASS; /*! 自クラスの型 */                                       \
                                                                                           \
	/*! 型情報 */                                                                       \
	[[deprecated( "BP_OBJECT_TYPE() と BP_OBJECT_BASE_TYPE() "                            \
				  "は古い宣言です。BP_OBJECT_DECL(クラス名, u8\"解説文\") " \
				  "に置換してください。" )]] static ClassObjectType<CLASS>                 \
		Type;                                                                              \
                                                                                           \
	/*! 型情報を取得 */                                                              \
	virtual const ObjTypeInfo* typeInfo() const                                            \
...
#define BP_OBJECT_BASE_IMPL( CLASS, DESC_NAME ) \
	...
	ClassObjectType<CLASS>                      \
		CLASS::Type( #CLASS, sizeof( CLASS ), nullptr, DESC_NAME );
...
public:
	...
	//! @param hitInfo ヒット情報
	virtual void
	OnHit( [[maybe_unused]] const ComponentCollision::HitInfo& hit_info )
	{
...
#pragma warning( disable: 26813 )
		// ここは&で参照すべき所ではない
		if( hit_info.hit_collision_->GetCollisionGroup() ==
			ComponentCollision::CollisionGroup::GROUND )
		{
			hit_info.collision_->SetCollisionStatus(
...
#pragma warning( default: 26813 )
	...
	//! @param hitInfo ヒット情報
	virtual void
	OnHitPhysics( [[maybe_unused]] const ComponentPhysics::HitInfo& hit_info )
	{
	}
	...
	virtual const matrix GetWorldMatrix() const override
	{
		...
		assert(
			cmp && "このオブジェクトは、ComponentTransformが存在していません。"
				   "位置移動はできません"
		);
...
std::shared_ptr<T> Object::AddComponent( Args... args )
{
	assert(
		this != nullptr &&
		"実行しているオブジェクト(this)"
		"がありません。「再試行」をおして、「呼び出し履歴」からどこでemptyにな"
		"ったのかを確認してください。"
	);
