...
public:
	...
	public:
		...
		//! 優先を設定変更します
		void
		SetPriority( ObjectPtr obj, ProcTiming timing, ProcPriority priority );

		//! 優先を設定変更します
		void SetPriority(
			ComponentPtr commponent,
			ProcTiming	 timing,
		...
		GetObjectsPtr( const std::string_view name = "" );

		template<class T>
		std::shared_ptr<T>
		GetObjectPtrWithCreate( const std::string_view name = "" );
	...
	struct Object
	{
		...
		Create(
			const std::string_view name			= u8"object",
		...
		//! @param draw 描画優先
		template<class T>
		static std::shared_ptr<T> CreateDelayInitialize(
			const std::string_view name			= u8"object",
			bool				   no_transform = false,
			ProcPriority		   update		= ProcPriority::NORMAL,
			ProcPriority		   draw			= ProcPriority::NORMAL
	...
	{
		return Object::CreateDelayInitialize<T>(
			name,
			no_transform,
			update,
			draw
...
template<class T>
std::vector<std::shared_ptr<T>>
Scene::Base::GetObjectsPtr( const std::string_view name )
{
...
template<class T>
std::shared_ptr<T>
Scene::Base::GetObjectPtrWithCreate( const std::string_view name )
{
