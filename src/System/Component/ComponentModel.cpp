...
namespace
{
	...
	)
	{
		...
		const char** tbl =
			(const char**)malloc( items_count * sizeof( char* ) );
		if( tbl != nullptr )
		{
			for( int i = 0; i < items_count; i++ )
				tbl[i] = items[i].data();

			value_changed = ImGui::ListBox(
				label,
				current_item,
				Items_ArrayGetter,
				(void*)tbl,
...
void ComponentModel::Draw()
{
	...
	auto drawFrame = [this, &empty]( int i )
	{
		...
		{
			// マテリアルが存在した
			model_->overrideTexture(
				Model::TextureType::Diffuse,
				materials_[i].diffuse_
			);
			model_->overrideTexture(
				Model::TextureType::Normal,
				materials_[i].normal_
			);
			model_->overrideTexture(
				Model::TextureType::Roughness,
				materials_[i].roughness_
			);
			model_->overrideTexture(
				Model::TextureType::Metalness,
				materials_[i].metalness_
			);
			model_
				->overrideTexture( Model::TextureType::AO, materials_[i].AO_ );
			model_->overrideTexture(
				Model::TextureType::Specular,
				materials_[i].specular_
			);
		}
		else
		{
	...
	if( draw_meshes_.size() > 0 && draw_meshes_[0] == -1 )
	{
		if( materials_.find( -1 ) != materials_.end() )
		{
			model_->overrideTexture(
				Model::TextureType::Diffuse,
				materials_[-1].diffuse_
			);
			model_->overrideTexture(
				Model::TextureType::Normal,
				materials_[-1].normal_
			);
			model_->overrideTexture(
				Model::TextureType::Roughness,
				materials_[-1].roughness_
			);
			model_->overrideTexture(
				Model::TextureType::Metalness,
				materials_[-1].metalness_
			);
			model_
				->overrideTexture( Model::TextureType::AO, materials_[-1].AO_ );
			model_->overrideTexture(
				Model::TextureType::Specular,
				materials_[-1].specular_
			);
			model_->render();
		}
		else
...
void ComponentModel::GUI()
{
	...
	ImGui::Begin( obj_name.data() );
	{
		...
		// モデルコンポーネント表示
		auto name = std::string( u8"Model 【" ) + std::string( GetName() ) +
					std::string( u8"】" );
			...
			if( !models.empty() )
			{
				...
				if( ImGui::TreeNode( UNIQUE_TEXT( "Attach Node" ) ) )
				{
					if( editor_attach_model_index_ >= 0 &&
						editor_attach_node_index_ >= 0 )
					{
						auto node_list =
							model_vec[editor_attach_model_index_]->GetNodesName(
							);

						bool attached =
							model_status_.is( ModelBit::AttachedOtherModel );
						if( ImGui::Checkbox(
								UNIQUE_TEXT( u8"モデルに接続" ),
								&attached
							) )
						{
							model_status_
								.set( ModelBit::AttachedOtherModel, attached );
							if( attached )
							{
								auto model_name =
									model_name_vec[editor_attach_model_index_];
								Attach(
									model_name,
									node_list[editor_attach_node_index_]
								);
							}
							else
							{
					...
					if( editor_attach_model_index_ >= 0 &&
						editor_attach_model_index_ < model_name_vec.size() )
						model_name = model_name_vec[editor_attach_model_index_];

					// ImGui::ListBox( UNIQUE_TEXT( u8"target model" ),
					// &editor_attach_model_index_, model_name_vec, 5 );
					editor_attach_model_index_ = Combo(
						UNIQUE_TEXT( u8"target model" ),
						model_name,
						model_name_vec
					);
					if( editor_attach_model_index_ >= 0 )
					{
						editor_attach_model_ =
							model_vec[editor_attach_model_index_];
						auto node_list				   = editor_attach_model_->GetNodesName();
						editor_attach_node_manipulate_ = true;
						if( ListBox(
								UNIQUE_TEXT( u8"ノード名" ),
								&editor_attach_node_index_,
								node_list,
								4
							) )
						{
	...
	{
		matrix matx = MV1GetFrameLocalWorldMatrix(
			editor_attach_model_->GetModel(),
			editor_attach_node_index_
		);
		auto trns = GetOwner()->GetComponent<ComponentTransform>();
...
void ComponentModel::PlayAnimationNoSame(
...
void ComponentModel::PlayAnimation(
	std::string_view name,
	bool			 loop,
	float			 blend_time,
	float			 start_time
...
bool ComponentModel::Attach(
	const std::string_view& model,
	const std::string_view& node,
	bool					use_model_node_scale
)
{
	auto owner = GetOwner();
	{
