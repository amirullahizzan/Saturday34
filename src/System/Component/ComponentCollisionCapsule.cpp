...
void ComponentCollisionCapsule::Init()
{
	...
	listener_.SetComponent(
		std::dynamic_pointer_cast<ComponentCollisionCapsule>( shared_from_this()
		)
	);
...
void ComponentCollisionCapsule::PrePhysics()
{
	...
	if( IsCollisionStatus( CollisionBit::UsePhysics ) )
	{
		// Physicsでの移動
		vec =
			mat._41_42_43 - mul( Matrix(), owner->GetWorldMatrix() )._41_42_43;
...
void ComponentCollisionCapsule::Draw()
{
	...
	mat =
		mul( mat, matrix::translate( float3{ 0, half_height + radius_, 0 } ) );
	mat = mul( mat, Matrix() );
...
void ComponentCollisionCapsule::GUI()
{
	...
	ImGui::Begin( obj_name.data() );
	{
		...
		if( ImGui::TreeNode( ui_name.c_str() ) )
		{
			...
			ImGui::DragFloat3(
				( colname + u8"座標(T)" ).c_str(),
				matrixTranslation,
				0.01f
			);
			ImGui::DragFloat3(
				( colname + u8"回転(R)" ).c_str(),
				matrixRotation,
				0.1f
			);
