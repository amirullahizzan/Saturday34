...
void ComponentCollisionLine::GUI()
{
	...
	ImGui::Begin( obj_name.data() );
	{
		...
		if( ImGui::TreeNode( ui_name.c_str() ) )
		{
			...
			bool change	 = false;
			change		|= ImGui::DragFloat3(
				 ( colname + u8" Start" ).data(),
				 (float*)&pos1,
				 0.1f
			 );
			change |= ImGui::DragFloat3(
...
//! @return 自分のSharedPtr
ComponentCollisionLinePtr
ComponentCollisionLine::SetLine( float3 start, float3 end )
{
	auto mat = HelperLib::Math::CreateMatrixByFrontVector(
		end - start,
		{ 0, 1, 0 },
		true
	);
...
ComponentCollisionLine::IsHit( ComponentCollisionPtr col )
{
	...
	case ComponentCollision::CollisionType::SPHERE:
		return isHit(
			std::dynamic_pointer_cast<ComponentCollisionLine>( shared_from_this(
			) ),
