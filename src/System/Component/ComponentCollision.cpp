...
)
{
	...
	{
		cpos =
			mul( float4( col1->GetTranslate(), 1 ), col1->attach_node_matrix_ )
				.xyz;
		opos = cpos;
	}
	else
	...
	// 当たりの確認(通常)
	{
		...
		hit_poly_dim = MV1CollCheck_Capsule(
			mdl->GetModel(),
			-1,
			cast( ocenter ),
			cast( ncenter ),
		...
		for( int i = 0; i < hit_poly_dim.HitNum; i++ )
		{
			...
			// カプセルへの戻し方向
			if( HelperLib::Math::NearlyEqual(
					length( line_pos - tri_pos ),
					0
				) )
			{
...
//! @return 当たり情報
ComponentCollision::HitInfo ComponentCollision::isHit(
	ComponentCollisionModelPtr col1,
...
)
{
	...
	else
	{
		...
		opos1 =
			mul( float4(
					 col1->GetTranslate() + float3( 0, col1->GetHeight(), 0 ),
					 1
				 ),
				 col1->GetOwner()->GetOldWorldMatrix() )
				.xyz;
...
ComponentCollision::HitInfo ComponentCollision::isHit(
	ComponentCollisionLinePtr	col1,
	ComponentCollisionSpherePtr col2
)
{
	// 当たり情報
	...
	if( len <= radius )
	{
		...
		info.hit_ = true;
		info.hit_position_ =
			point + vec * acos( len / radius ) / 0.5f * DX_PI_F;
	}
...
)
{
	...
	info.hit_position_ = float3(
		hit_poly.HitPosition.x,
