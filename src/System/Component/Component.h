...
public:
	ComponentTypeInfo(
		...
		size_t			   class_size,
		ComponentTypeInfo* parent_type = nullptr,
	...
	ComponentPtr tmp =
		std::shared_ptr<class Component>( (Component*)createInstance() );
...
#define BP_COMPONENT_DECL( CLASS, ... )                                                                              \
	...
	using ComponentClass =                                                                                           \
		CLASS; /*! 自クラスの型 : これ以降は親クラスではなく自クラスを指す */              \
                                                                                                                     \
	/*! 型情報 */                                                                                                 \
	static inline ClassComponentType<CLASS> Type =                                                                   \
		ClassComponentType<CLASS>( #CLASS, &Super::Type, __VA_ARGS__ );                                              \
                                                                                                                     \
	/*! 型情報を取得 */                                                                                        \
...
#define BP_COMPONENT_BASE_TYPE( CLASS )                                                       \
	using MyClass = CLASS; /*! 自クラスの型 */                                          \
                                                                                              \
	/*! 型情報 */                                                                          \
	[[deprecated( "BP_COMPONENT_TYPE() と BP_COMPONENT_BASE_TYPE() "                         \
				  "は古い宣言です。BP_COMPONENT_DECL(クラス名, u8\"解説文\") " \
				  "に置換してください。" )]] static ClassComponentType< CLASS>                \
		Type;                                                                                 \
                                                                                              \
	/*! 型情報を取得 */                                                                 \
	virtual const ComponentTypeInfo* typeInfo() const                                         \
...
// ClassComponentType<CLASS> CLASS::Type(#CLASS, sizeof(CLASS), &Super::Type,
// DESC_NAME);
...
#define UNIQUE_TEXT( n ) UniqueText( n ).c_str()
	...
	std::string UniqueText( const std::string_view& name, int id = 0 )
	{
		auto str =
			...
			std::to_string( id ) + "." + std::to_string( (size_t)( this ) );
		return str;
	}
