...
public:
	...
	bool Attach(
		...
		bool					use_model_node_scale = false
	);
...
public:
	...
	struct Material
	{
		...
		Material(
			...
			const std::string_view roughness,
...
private:
	...
	ShaderPs*						  overrided_shader_ps_ =
		nullptr;	//!< 上書きするピクセルシェーダー
...
private:
	...
	CEREAL_SAVELOAD( arc, ver )
	{
		...
		if( ver >= 1 )
		{
			...
			arc( cereal::make_nvp(
				"use_model_node_scale",
				use_model_node_scale_
			) );
		}
...
CEREAL_SAVELOAD_OTHER( Animation::Desc, arc, other )
{
	arc( CEREAL_NVP( other.name_ ),			//!< アニメーション名(任意)
		 ...
		 CEREAL_NVP( other.animation_index_
		 ),	   //!< ファイル内のアニメーション番号
		 CEREAL_NVP( other.animation_speed_
