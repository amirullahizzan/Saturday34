...
// 共通の設定
	...
	public:
		...
		virtual JPH::ValidateResult OnContactValidate(
			...
			[[maybe_unused]] const JPH::CollideShapeResult& collision_result
		) override
		{
			...
			// broad_phase_layer_interfaceでやるべきことをここで行う todo
			// broad_phase_layer_interfaceの変更
			if( !( ComponentPhysics::GetHitCollision( ct1 ) & (u16)ct2 ) ||
				!( ComponentPhysics::GetHitCollision( ct2 ) & (u16)ct1 ) )
			{
				// ヒットしない
			...
			// どちらかの指定でオーバーラップするか?
			if( pd1->GetOverlapCollision() & (u16)ct2 ||
				pd2->GetOverlapCollision() & (u16)ct1 )
			{
				return JPH::ValidateResult::RejectAllContactsForThisBodyPair;
			}
	...
	class JPH_DEBUG_RENDERER_EXPORT BPDebugRenderer final
	...
	public:
		...
		JPH_INLINE void*
		operator new( size_t inCount, std::align_val_t inAlignment )
		{
			return JPH::AlignedAllocate(
				inCount,
				static_cast<size_t>( inAlignment )
			);
		}

		JPH_INLINE void operator delete(
			void*			 inPointer,
			std::align_val_t inAlignment
		) noexcept
		{
			inAlignment;	// no use...
	...
	public:
		...
		public:
			...
			BatchImpl( const Triangle* inTriangles, int inTriangleCount )
			{
				...
				polygons = new PolygonData(
					(const Vertex*)inTriangles,
					inTriangleCount * 3
				);
				// memcpy( polygons->vertex, inTriangles, sizeof( Triangle ) *
				// inTriangleCount );
			}
			...
			)
			{
				if( inVertices == nullptr || inIndices == nullptr ||
					inVertexCount <= 0 || inIndexCount <= 0 )
					return;
				...
				// memcpy( polygons->vertex, inVertices, sizeof( Vertex ) *
				// inVertexCount ); memcpy( polygons->index, inIndices, sizeof(
				// JPH::uint ) * inIndexCount );
			}
		...
		Batch
		CreateTriangleBatch( const Triangle* inTriangles, int inTriangleCount )
			override
		{
			if( inTriangles == nullptr || inTriangleCount == 0 )
				return nullptr;
		...
		) override
		{
			if( inVertices == nullptr || inVertexCount <= 0 ||
				inIndices == nullptr || inIndexCount <= 0 )
				return nullptr;
...
void ComponentPhysics::Init()
{
	...
	// ProcTiming::PostUpdate, (Priority)((int)Priority::HIGHEST+100) );
}
...
void ComponentPhysics::Draw()
{
	...
	case(int)JPH::EShapeSubType::Box:
		...
		if( debug_renderer )
			debug_renderer->DrawWireBox(
				mat,
				JPH::AABox(
					JPH::Vec3{ -shape_data.box_size.x,
							   -shape_data.box_size.y,
							   -shape_data.box_size.z },
					JPH::Vec3{ shape_data.box_size.x,
							   shape_data.box_size.y,
							   shape_data.box_size.z }
				),
	...
	case(int)JPH::EShapeSubType::Sphere:
		...
		// DrawSphere3D( cast( pos ), shape_data.sphere_radius, 8,
		// physics_color, GetColor( 0, 0, 0 ), FALSE );
		if( debug_renderer )
			debug_renderer->DrawWireSphere(
				{ pos.x, pos.y, pos.z },
				shape_data.sphere_radius,
				JPH::Color( physics_color )
			);
		break;
...
void ComponentPhysics::GUI()
{
	...
	if( ImGui::TreeNode( u8"Physics" ) )
	{
		...
		// 形状
		{
			...
			if( body_ )
			{
				auto ref = physics::Engine::bodyInterface()->GetShape(
					GetBodyID( body_ )
				);
				if( ref )
					shape = (int)ref->GetSubType();
			...
			if( ImGui::Combo( u8"型(Shape)", &shape, items, 6 ) )
			{
				...
				if( shape != shape_old )
				{
					...
					case 0:
						...
						body_ = nullptr;
						if( constraint_ )
							physics::Engine::physicsSystem()->RemoveConstraint(
								constraint_
							);
...

			{
...
#endif
				{
					...
					case 1:
						// to BOX
						value_change |= ImGui::DragFloat3(
							u8"大きさ(XYZ size)",
							(float*)&shape_data.box_size,
							0.5f
						);
					...
					case 2:
						...
						value_change |=
							( /* ImGui::DragFloat3( "sphere center",
								 (float*)&shape_data.sphere_center, 0.1f ) || */
							  ImGui::DragFloat(
								  u8"半径(radius)",
								  &shape_data.sphere_radius,
								  0.1f,
			...
			if( body_ )
			{
				...
				if( ImGui::DragFloat3(
						UNIQUE_TEXT( u8"移動方向(LinearVelocity)" ),
						(float*)&linear,
						0.1f
					) )
				...
				{
					body_->setGravityFactor( gravity_factor_ );
					physics::Engine::bodyInterface()->ActivateBody(
						GetBodyID( body_ )
					);
				}
				else
				{
					body_->setGravityFactor( 0 );
					physics::Engine::bodyInterface()->DeactivateBody(
						GetBodyID( body_ )
					);
				}
				...
				// ImGui::CheckboxFlags( u8"初期化済", &val, 1 <<
				// (int)PhysicsBit::Initialized );
				bool refresh = ImGui::CheckboxFlags(
					u8"X軸回転禁止",
					bit,
					1 << (int)PhysicsBit::LockRotX
				);
				refresh |= ImGui::CheckboxFlags(
					u8"Y軸回転禁止",
					bit,
					1 << (int)PhysicsBit::LockRotY
				);
				refresh |= ImGui::CheckboxFlags(
					u8"Z軸回転禁止",
					bit,
					1 << (int)PhysicsBit::LockRotZ
				);

				if( refresh )
				{
					LockRotateAxis(
						GetPhysicsStatus( PhysicsBit::LockRotX ),
...
void ComponentPhysics::LockRotateAxis( bool x, bool y, bool z )
{
	...
	constraint_ =
		(JPH::SixDOFConstraint*)physics::Engine::bodyInterface()
			->CreateConstraint(
				sSettings,
				JPH::Body::sFixedToWorld.GetID(),
				GetBodyID( body_ )
			);
...
bool ComponentPhysics::CreateSphere(
	float3 center,
	float  radius,
	float  density
)
...
bool ComponentPhysics::CreateCapsule(
	float half_height,
	float radius,
	float density
)
...
bool ComponentPhysics::CreateCylinder(
	float half_height,
	float radius,
	float density
)
...
bool ComponentPhysics::CreateMesh( ComponentModelPtr model, float scale )
{
	...
	body_ = physics::createRigidBody(
		shape::Mesh{ model->GetModelClass(), scale },
