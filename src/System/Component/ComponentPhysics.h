...
public:
	...
	inline void
	SetPhysicsStatus( PhysicsBit b, bool on )	 //!< ステータスの設定
	{
	...
	bool CreateSphere(
		float3 center,
		float  radius,
		float  density = shape::default_density
	);
	...
	//! @param hit_info
	virtual void
	OnHit( [[maybe_unused]] const ComponentPhysics::HitInfo& hit_info )
	{
	}
	...
	//! @param hit_info
	virtual void
	OnBeginOverlap( [[maybe_unused]] const ComponentPhysics::HitInfo& hit_info )
	{
	}
	...
	//! @param hit_info
	virtual void
	OnEndOverlap( [[maybe_unused]] const ComponentPhysics::HitInfo& hit_info )
	{
	}
