...
public:
	...
	ComponentObjectControllerPtr [[deprecated(
		"SetSpeed()は古い形式です。SetMoveSpeed()を使用してください"
	...
	SetKeys( int up, int down, int left, int right );

	ComponentObjectControllerPtr
	SetCameraKeys( int up, int down, int left, int right );
	...
	const float [[deprecated(
		"GetSpeed()は古い形式です。GetMoveSpeed()を使用してください"
	)]] GetSpeed() const
	{
		return GetMoveSpeed();
	}
