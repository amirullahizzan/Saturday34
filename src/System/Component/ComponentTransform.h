...
extern void DecomposeMatrixToComponents(
	const float* matx,
	float*		 translation,
	float*		 rotation,
	float*		 scale
);
extern void RecomposeMatrixFromComponents(
...
public:
	...
	//! @param up_change Y軸も変更するか?
	//! @return 自分のSharedPtr
	auto SetRotationToVectorOnParent(
		float3 vec,
		float3 up		 = { 0, 1, 0 },
		bool   up_change = false
	)
	...
	//! @param up_change Y軸も変更するか?
	//! @return 自分のSharedPtr
	auto SetRotationToPositionWithLimit(
		float3 position,
		float  limit,
		float3 up		 = { 0, 1, 0 },
		bool   up_change = false
	)
	{
		...
		float3 vec = position - GetMatrix().translate();
		return SetRotationToVectorOnParentWithLimit(
			vec,
			limit,
			up,
			up_change
	...
	//! @param up_change Y軸も変更するか?
	//! @return 自分のSharedPtr
	auto SetRotationToVectorWithLimit(
		float3 vec,
		float  limit,
		float3 up		 = { 0, 1, 0 },
		bool   up_change = false
	...
	//! @param up_change Y軸も変更するか?
	//! @return 自分のSharedPtr
	auto SetRotationToVectorOnParentWithLimit(
		float3 vec,
		float  limit,
		float3 up		 = { 0, 1, 0 },
		bool   up_change = false
