...
namespace
{
	constexpr unsigned int MAX_PAD_NUM =
		static_cast<unsigned int>( PAD_NO::PAD_NO_MAX );	//<! PAD接続最大数
	constexpr unsigned int MAX_PAD_TYPE =
		static_cast<unsigned int>( PAD_ID::PAD_ID_MAX
		);	  //<! 上下左右/A B C X Y Z L R START Mボタン
	constexpr unsigned int MIN_PAD_TYPE =
		static_cast<unsigned int>( PAD_ID::PAD_A
		);											  //<! 上下左右/A B C X Y Z L R START Mボタン
	constexpr unsigned int MAX_PAD_DIRECT_NUM = 4;	  //<! 方向キーの方向最大値
	constexpr float		   MAX_ANALOG_INPUT_VALUE =
		1000.0f;	//<! アナログスティックの入力値の最大値
	constexpr unsigned int PAD_BUTTON_NUM =
		static_cast<unsigned int>( PAD_ID::PAD_R_PUSH ) -
		static_cast<unsigned int>( PAD_ID::PAD_A ) + 1;

	std::vector<DINPUT_JOYSTATE>
		pad_input_states;	 //<! 詳細なパッドの状態の取得
	std::array<bool, MAX_PAD_NUM>
		use_pads;	 //<! 使用しているパッドの状態を管理
...
//    //	//int deg = min(((pov_0 >= 31500) ? 35000 : 36000) / (pov_0 + 4500),
//    4) % 4;
//    //	//printfDx("POV0: %d(%d)\n", pov_0, deg);
...
bool IsPadOn( PAD_ID pad_id, PAD_NO pad_no )
{
	...
	// int             pov_0_index = ((input_state.POV[0] + 4500) % 36000) /
	// 9000;
	unsigned int direct_id =
		static_cast<unsigned int>( pad_id ) -
		static_cast<unsigned int>( PAD_ID::PAD_D_UP );
