...
namespace
{
	...
	auto BindObject( ProcTiming proc, ObjectPtr obj )
	{
		...
		assert(
			static_cast<u32>( proc ) < static_cast<u32>( ProcTiming::NUM )
		);
...
)
{
	...
	auto& proc_update = obj->GetProc(
		GetProcTimingName( ProcTiming::Update ),
		ProcTiming::Update
	);
...
)
{
	...
	auto& proc_preupdate = obj->GetProc(
		GetProcTimingName( ProcTiming::PreUpdate ),
	...
	auto& proc_late_update = obj->GetProc(
		GetProcTimingName( ProcTiming::LateUpdate ),
	...
	auto& proc_pre_physics = obj->GetProc(
		GetProcTimingName( ProcTiming::PrePhysics ),
	...
	auto& proc_post_physics = obj->GetProc(
		GetProcTimingName( ProcTiming::PostPhysics ),
		ProcTiming::PostPhysics
	);
	...
	auto& proc_postupdate = obj->GetProc(
		GetProcTimingName( ProcTiming::PostUpdate ),
	...
	auto& proc_predraw = obj->GetProc(
		GetProcTimingName( ProcTiming::PreDraw ),
		ProcTiming::PreDraw
	);
	...
	auto& proc_latedraw = obj->GetProc(
		GetProcTimingName( ProcTiming::LateDraw ),
		ProcTiming::LateDraw
	);
	...
	auto& proc_postdraw = obj->GetProc(
		GetProcTimingName( ProcTiming::PostDraw ),
		ProcTiming::PostDraw
	);
...
void Scene::SetNextScene( BasePtr scene )
{
	...
	if( scene && Base::IsSceneExist( scene ) )
	{
		...
		// TODO log/ensure処理
		assert(
			0 && "すでに同じタイプのシーンが用意されています。前のシーンは「無"
				 "視」を押すことで削除して進めることができます"
		);
...
void Scene::PreUpdate()
{
	...
	if( current_scene_ )
	{
		if( !current_scene_->GetStatus( Scene::Base::StatusBit::Initialized ) )
		{
			...
			current_scene_
				->SetStatus( Scene::Base::StatusBit::Initialized, initialized );
		...
		for( auto obj: current_scene_->pre_objects_ )
		{
			...
			auto& proc_draw = obj->GetProc(
				GetProcTimingName( ProcTiming::Draw ),
				ProcTiming::Draw
			);

			current_scene_
				->Register( obj, proc_update.priority_, proc_draw.priority_ );
		}
		// 仮登録のクリア
		...
		if( !current_scene_->GetStatus( Scene::Base::StatusBit::Serialized ) )
		{
			...
			current_scene_
				->SetStatus( Scene::Base::StatusBit::Serialized, true );
		}
...
void Scene::GUI()
{
	...
	ImGui::Begin( name.c_str(), (bool*)nullptr, flags );
	{
		...
		if( ImGui::Button( u8"作成 (Scene::Object::Create)" ) &&
			object_type != nullptr )
		{
	...
	for( int i = 0; i < size; i++ )
	{
		...
		if( obj->GetStatus( ::Object::StatusBit::ShowGUI ) )
		{
			...
			obj->GUI();
			assert(
				"継承先のGUI()にて__super::GUI()を入れてください." &&
