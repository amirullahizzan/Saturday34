...
namespace physics
{
	...
	public:
		...
		//  コンタクトリスナーを設定
		virtual void setListener( physics::Character::ContactListener* listener
		) override;
		...
		//  速度を設定 (単位:m/s)
		virtual void setLinearVelocity( const float3& linear_velocity
		) override;
		...
		virtual float3 move(
			...
			const float3& jump_vector = float3( 0.0f, 0.0f, 0.0f )
		) override;
		...
		virtual bool walkStairs(
			...
			const float3& step_down_extra = float3( 0.0f, 0.0f, 0.0f )
		) override;
		...
		virtual bool setShape(
			...
			const float3&	   shape_offset = float3( 0.0f, 0.0f, 0.0f )
		) override;
	...
	//---------------------------------------------------------------------------
	void
	CharacterImpl::setListener( physics::Character::ContactListener* listener )
	{
		listener_ = listener;
	}
	...
	)
	{
		...
		if( ground_state == physics::Character::GroundState::
								OnGround	// 地面に接地している時
			&& ( relative_vertical_velocity < 0.1f
			   ) )	  // かつ、地面から離れようとしていない時
		{
			// 移動床の速度を新しい速度にする
	...
	//! キャラクターの形状を入れ替える（しゃがみ、構え方など）
	//---------------------------------------------------------------------------
	bool CharacterImpl::setShape(
		const shape::Base& shape,
		const float3&	   shape_offset
	)
	{
		assert(
			shape.shapeType() == shape::Type::Capsule &&
			"カプセル以外は未対応です."
		);
