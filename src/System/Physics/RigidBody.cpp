...
namespace physics
{
	...
	public:
		...
		)
		{
			...
			// ワールドに追加 (アクティブ化)
			physics::Engine::bodyInterface()
				->AddBody( jph_body_->GetID(), JPH::EActivation::Activate );
		...
		virtual void
		setPosition( const float3& position, bool is_activate = true ) override
		{
		...
		//! true:アクティブにする false:アクティブにしない
		virtual void
		setRotation( const quaternion& rot, bool is_activate = true ) override
		{
		...
		{
			return castJPH(
				physics::Engine::bodyInterface()->GetWorldTransform( body_id_ )
			);
		}
		...
		{
			return castJPH( physics::Engine::bodyInterface()
								->GetCenterOfMassTransform( body_id_ ) );
		}
		...
		) override
		{
			...
			physics::Engine::bodyInterface()
				->MoveKinematic( body_id_, tpos, trot, delta_time );
		}
		...
		{
			physics::Engine::bodyInterface()
				->SetLinearVelocity( body_id_, castJPH( v ) );
		}
		...
		{
			return castJPH(
				physics::Engine::bodyInterface()->GetLinearVelocity( body_id_ )
			);
		}
		...
		{
			physics::Engine::bodyInterface()
				->AddLinearVelocity( body_id_, castJPH( v ) );
		}
		...
		{
			physics::Engine::bodyInterface()
				->SetAngularVelocity( body_id_, castJPH( vrot ) );
		}
		...
		{
			return castJPH(
				physics::Engine::bodyInterface()->GetAngularVelocity( body_id_ )
			);
		}
		...
		{
			physics::Engine::bodyInterface()
				->AddForce( body_id_, castJPH( force ) );
		}
		...
		//! @param  [in]    point   位置
		virtual void
		addForce( const float3& force, const float3& point ) override
		{
			physics::Engine::bodyInterface()
				->AddForce( body_id_, castJPH( force ), castJPH( point ) );
		}
		...
		{
			physics::Engine::bodyInterface()
				->AddTorque( body_id_, castJPH( torque ) );
		}
		...
		{
			physics::Engine::bodyInterface()
				->AddImpulse( body_id_, castJPH( impulse ) );
		}
		...
		//! @param  [in]    point   位置
		virtual void
		addImpulse( const float3& impulse, const float3& point ) override
		{
			physics::Engine::bodyInterface()
				->AddImpulse( body_id_, castJPH( impulse ), castJPH( point ) );
		}
		...
		{
			physics::Engine::bodyInterface()
				->AddAngularImpulse( body_id_, castJPH( impulse ) );
		}
		...
		{
			physics::Engine::bodyInterface()
				->SetGravityFactor( body_id_, coeff );
		}
		...
		{
			return castJPH(
				physics::Engine::bodyInterface()->GetInverseInertia( body_id_ )
			);
		}
	...
	)
	{
		...
		// 作成
		return std::make_shared<RigidBodyImpl>(
			result.Get(),
			layer,
			motion_type
		);
	...
	createRigidBody( const shape::Mesh& o, u16 layer )
	{
		//----------------------------------------------------------
		// 頂点配列を取得
		//----------------------------------------------------------
		...
		JPH::MeshShapeSettings
										shape_settings( vertexList, indexList, materialList );
		JPH::ShapeSettings::ShapeResult result = shape_settings.Create();
