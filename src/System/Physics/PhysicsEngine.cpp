...
namespace
{
	...
	public:
		...
		virtual bool
		ShouldCollide( JPH::ObjectLayer layer1, JPH::ObjectLayer layer2 )
			const override
		{
			// カスタム関数がある場合は実行
			if( BroadPhaseLayers::can_object_layer_collide_ )
			{
				return BroadPhaseLayers::can_object_layer_collide_(
					layer1,
					layer2
				);
			}
	...
	public:
		...
		//! Broad-phaseレイヤーを取得
		virtual JPH::BroadPhaseLayer GetBroadPhaseLayer( JPH::ObjectLayer layer
		) const override
		{
		...
		virtual const char* GetBroadPhaseLayerName( JPH::BroadPhaseLayer layer
		) const override
		{
	...
	public:
		...
		//! @retval true    衝突なし
		virtual bool
		ShouldCollide( JPH::ObjectLayer layer1, JPH::BroadPhaseLayer layer2 )
			const override
		{
	...
	public:
		...
		virtual void OnBodyActivated(
			...
			[[maybe_unused]] JPH::uint64		body_user_data
		) override
		{
...
namespace physics
{
	...
	public:
		...
		virtual void
		overrideLayerCollide( std::function<bool( u16, u16 )> callback
		) override;
	...
	EngineImpl::EngineImpl()
	{
		...
		//----------------------------------------------------------
		{
			...
			{
				body_interface_ = &jph_physics_system_->GetBodyInterfaceNoLock(
				);	  // Lockなし(利用には注意)
			}
		}
	...
	void EngineImpl::optimize()
	{
		...
		);	  // ※このタイミングではまだオブジェクトが1つもないため無意味です。
	}
	...
	//---------------------------------------------------------------------------
	void
	EngineImpl::overrideLayerCollide( std::function<bool( u16, u16 )> callback )
	{
