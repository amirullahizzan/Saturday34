...
namespace physics
{
	...
	public:
		...
		virtual void
		setPosition( const float3& position, bool is_activate = true ) = 0;
		...
		//! true:アクティブにする false:アクティブにしない
		virtual void
		setRotation( const quaternion& rot, bool is_activate = true ) = 0;
		...
		//! @param  [in]    point   位置
		virtual void
		addImpulse( const float3& impulse, const float3& point ) = 0;
