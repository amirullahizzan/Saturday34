...
namespace HelperLib::File
{
	...
	Files_t
	GetFilesFromDirectory( std::string_view dirname, std::string_view ext )
	{
		Files_t							 vfiles;
...
namespace HelperLib::OS
{
	...
	{
		std::string
		GetRegString( HKEY hkey, const char* sub_key, const char* value_name )
		{
			DWORD key_type	= 0;
			DWORD data_size = 0;
			if( ERROR_SUCCESS ==
...
namespace HelperLib::Math
{
	...
	matrix
	LookAtMatrixForObject( float3 my_object_pos, float3 target_object_pos )
	{
		auto mat =
			CreateMatrixByFrontVector( target_object_pos - my_object_pos );
